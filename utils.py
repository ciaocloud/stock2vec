...
#     prices = get_df(st, et)
    lookback = 24
#     sop = compute_sop(df, lookback=lookback, gen_plot=True)
#     wpR = compute_williams(df, lookback=lookback, gen_plot=True)
    
    prices = df['Close']
#     print(prices.index)
#     sma = compute_sma(prices, lookback=lookback, gen_plot=True)
#     print(sma)
    
#     bbp = compute_bbp(prices, lookback=lookback, gen_plot=True)
# #     print bbp


    symbol = 'BTC'
    prices_df = prices.to_frame(name=symbol)
#     prices_df = pd.DataFrame(prices, columns=[symbol])
#     print(prices_df)
#     
    benchmark_orders = pd.DataFrame(0, index=prices_df.index, columns=[symbol])
    benchmark_orders.ix[0] = 1000
    t0 = time.time()
    benchmark_vals = compute_port_vals(prices_df, benchmark_orders, start_val=sv)
    print( compute_port_stats(benchmark_vals) )
    print( "Elasped time = {} s for benchmark simulation".format(time.time() - t0) )
 
#     t0 = time.time()
#     cheat_orders = cheatingPolicy(prices_df, sv=sv)
#     cheat_vals = compute_port_vals(prices_df, cheat_orders, start_val=sv)
#     print( compute_port_stats(cheat_vals) )
#     print( "Elasped time = {} s for cheating policy generation and simulation".format(time.time() - t0) )
    
    t0 = time.time()
    tech_orders = technicalPolicy(df, sv=sv, lookback=lookback, symbol=symbol) 
    tech_vals = compute_port_vals(prices_df, tech_orders, start_val=sv)
    print( compute_port_stats(tech_vals) )
    print( "Elasped time = {} s for technical analysis policy generation and simulation".format(time.time() - t0) )    
 
    ax = benchmark_vals.plot(color='b', label='benchmark', title='Policy  Comparison')
#     cheat_vals.plot(color='k', label='cheating', ax=ax)
    tech_vals.plot(color='y', label='technical analysis', ax=ax)
# #     ax.set_xlim(min(dates), max(dates))
# #     ax.set_ylim(0, max(opt_vals))
    ax.set_xlabel('Date')
