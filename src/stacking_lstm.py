...
#         self.n_emb, self.n_cont = n_emb, n_cont
        ...
        layer_sizes = [n_emb+n_cont] + hidden_sizes + [out_size]
        ...
        self.ensemb = nn.Linear(rnn_output_size+out_size, out_size)
    ...
    def forward(self, x_cat, x_cont, x_seq, hidden):
        ...
        out_ft = self.fc_layers(x)

        h0, c0 = hidden
        out_ts, (h, c) = self.rnn(x_seq, (h0.detach(), c0.detach()))
        out_ts = self.fc_rnn(out_ts[:, -1, :])

        x = torch.cat([F.relu(out_ft), F.relu(out_ts)], 1)
        x = self.ensemb(x)
        return x
